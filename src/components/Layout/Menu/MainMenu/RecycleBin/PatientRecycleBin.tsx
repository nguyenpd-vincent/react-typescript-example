...
import { finAllTrashContact, restoreContact } from "@/services/menu";
...
const PatientRecycleBin: React.FC<Props> = ({ setMenuStatus }) => {
  const t = useLocale();

  const [dataPatient, setDataPatient] = useState<API.TrashContactStruct[]>([]);
  ...
  const fetchPatientData = async () => {
    const res = await finAllTrashContact(page, 10);
    setDataPatient(res?.data?.data ?? []);
    if (res?.data?.totalData) {
      setTotalPage(Math.ceil(res?.data?.totalData / 10));
    }
  };
  ...
  const handleSubmit = async () => {
    await restoreContact(selectedCheckboxes);
    fetchPatientData();
  };
          ...
          <div className="w-full h-[350px]">
            <table className="table-fixed w-full">
              <thead className="bg-[#eeeeee]">
                <tr className="grid grid-cols-contactTrash ">
                  ...
                  <td className="py-[10px] pl-[20px] font-bold">Nom</td>
                </tr>
              </thead>
              <tbody className="max-h-[300px] overflow-y-auto overflow-x-hidden block">
                {dataPatient.map((item, index) => (
                  <tr
                    className={`grid grid-cols-contactTrash py-[4px] m-[0] hover:bg-[#f6f6f6] ${
                  ...
                  >
                    ...
                    <td className="py-[2px] pl-[20px] text-center">
                      {item?.practitionerAbbr}
                    </td>
                    <td className="py-[2px] pl-[20px] text-center">
                      {item?.nbr}
                    </td>
                    <td className="py-[2px] pl-[20px] w-full ">{`${item?.lastname} ${item?.firstname}`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default PatientRecycleBin;
